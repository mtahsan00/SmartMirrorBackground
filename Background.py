...
class Calander(Frame):
    def __init__(self,parent,*args,**kwargs):
        Frame.__init__(self,parent,bg='black')
        self.calanderItem = ''
        self.calanderLable = Label(self, font=('Helvetica', 50), fg="white", bg="black")
        self.calanderLable.pack(side=TOP, anchor=N)
        self.getCalander()
    def getCalander(self):
        ...
        for event in events:
            ...
            if self.calanderItem != eventName:
                self.calanderItem = eventName
                self.calanderLable.config(text=eventName)
        self.after(100000, self.getCalander)
...
class FullScreen():
    ...
    def __init__(self):
        ...
        self.calanderThing.pack(side=TOP,anchor=N)
        #Message icon
        self.messagesThing = Messages(self.topFrame)
        self.messagesThing.pack(side=BOTTOM, anchor=S, pady=100)
        #clock
